...
- **مدیریت دارایی‌ها** — جدول تو در تو با افزودن/ویرایش/حذف کلاس و زیرآیتم، محاسبه خودکار قیمت طلا و USDT، مرتب‌سازی کلاس‌ها بر اساس ارزش کل یا درصد سبد (پیش‌فرض: درصد نزولی)
...
| فرم | react-number-format, react-select |

## مرتب‌سازی جدول دارایی‌ها

در صفحه **مدیریت دارایی‌ها**، ردیف‌های کلاس (سطح اصلی) قابل مرتب‌سازی هستند؛ زیرآیتم‌ها زیر کلاس والد خود باقی می‌مانند.

| ستون | کلید مرتب‌سازی | توضیح |
|------|----------------|--------|
| ارزش کل (تومان) | `total` | جمع ارزش زیرآیتم‌های هر کلاس |
| درصد از کل ثروت | `percent` | سهم هر کلاس از کل سبد (پیش‌فرض) |

- پیش‌فرض: مرتب‌سازی بر اساس **درصد** به صورت **نزولی** (بیشترین سهم اول)
- کلیک روی هدر ستون: جابه‌جایی بین صعودی و نزولی
- کلیک روی ستون دیگر: مرتب‌سازی بر همان ستون با ترتیب نزولی
- مقادیر به صورت عددی مقایسه می‌شوند؛ با تغییر نرخ زنده یا ویرایش دارایی‌ها، ترتیب بر اساس مقادیر به‌روز محاسبه می‌شود
- فلش بالا/پایین فقط روی ستون فعال نمایش داده می‌شود
